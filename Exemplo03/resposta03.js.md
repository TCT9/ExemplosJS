...
function computarVotos(array_strVotos){
    ...
    total = 0;
    array_strVotos.forEach((item) => {
        array_votos.push(+item);        //converter array de strings em array de inteiros
        total += +item;     //contar o total de votos
    });

    array_votos.forEach((item, index) => {
        //converte para percentual, com duas casas decimais e
        //salva o percentual de cada tipo de voto
        
        array_percentual[index] = parseFloat( (100*array_votos[index]/total).toFixed(2) );    
        // array_percentual[index] =  (100*array_votos[index]/total).toFixed(2) ;    
    });
