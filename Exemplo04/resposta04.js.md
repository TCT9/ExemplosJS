...
class Equacao2Grau {
    ...
    getRaiz() {
        let resultado;
        const delta = this.getDelta().result;
        if (delta < 0 ) {
            resultado = new Result(new Raiz(null),true,"Erro: Esta equação não tem raízes reais!", Tipos_de_Erro.deltaNegativo);
        }else{

            let const_raiz_delta = Math.sqrt(delta)/(2*this._A);
            let const_raiz_B = -this._B/(2*this._A);
            
            let x1 = parseFloat( (const_raiz_B + const_raiz_delta).toFixed(2) );
            let x2 = parseFloat( (const_raiz_B - const_raiz_delta).toFixed(2) );
